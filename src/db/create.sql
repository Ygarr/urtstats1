...
-- SEQ urt_map
CREATE SEQUENCE seq_urt_map;

-- SEQ urt_map_img
CREATE SEQUENCE seq_urt_map_img;
...
-- TABLE urt_map_type
CREATE TABLE urt_map_type (
  id        integer              PRIMARY KEY
, name      varchar(20)
, descr     varchar(50)
);

COMMENT ON TABLE urt_map_type        IS 'UrT map type constants.';
COMMENT ON COLUMN urt_map_type.id    IS 'Constant ID (Enum ordinal).';
COMMENT ON COLUMN urt_map_type.name  IS 'Constant name (Enum name).';
COMMENT ON COLUMN urt_map_type.descr IS 'Constant description.';

INSERT INTO urt_map_type (id, name, descr)
VALUES (0, 'TINY', 'Very small');

INSERT INTO urt_map_type (id, name, descr)
VALUES (1, 'SMALL', 'Small');

INSERT INTO urt_map_type (id, name, descr)
VALUES (2, 'MEDIUM', 'Medium');

INSERT INTO urt_map_type (id, name, descr)
VALUES (3, 'LARGE', 'Large');
...
, avg_kd_ratio    numeric(4,2)   DEFAULT 0
...
-- TABLE urt_map
CREATE TABLE urt_map (
  id              bigint         PRIMARY KEY
, name            varchar(40)    NOT NULL
, file_name       varchar(40)    NOT NULL
, map_type_id     int            REFERENCES urt_map_type (id)
, has_bots        boolean        DEFAULT false
, match_count     integer        DEFAULT 0

, CONSTRAINT urt_map_uk1 UNIQUE(file_name)
);
CREATE INDEX urt_map_idx_01 ON urt_map (map_type_id);
CREATE INDEX urt_map_idx_02 ON urt_map (file_name);

COMMENT ON TABLE urt_map                    IS 'UrT map.';
COMMENT ON COLUMN urt_map.id                IS 'Map unique ID.';
COMMENT ON COLUMN urt_map.name              IS 'Map name.';
COMMENT ON COLUMN urt_map.file_name         IS 'Map file name.';
COMMENT ON COLUMN urt_map.map_type_id       IS 'Map type ID ref.';
COMMENT ON COLUMN urt_map.has_bots          IS 'Map has bots flag.';
COMMENT ON COLUMN urt_map.match_count       IS 'Map match count.';


-- TABLE urt_map_img
CREATE TABLE urt_map_img (
  id              bigint         PRIMARY KEY  DEFAULT nextval('seq_urt_map_img')
, map_id          bigint         NOT NULL  REFERENCES urt_map (id)
, last_modified   timestamp      NOT NULL  DEFAULT CURRENT_TIMESTAMP
, content         bytea
);

CREATE INDEX urt_map_img_idx_01 ON urt_map_img (map_id);

COMMENT ON TABLE urt_map_img                IS 'UrT map image.';
COMMENT ON COLUMN urt_map_img.id            IS 'Map img unique ID.';
COMMENT ON COLUMN urt_map_img.last_modified IS 'Map img last modified time.';
COMMENT ON COLUMN urt_map_img.content       IS 'Map img content.';
